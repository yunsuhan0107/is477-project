...
def count_eco_line_length(pgn_moves: str) -> int:
    normalised = normalise_moves(pgn_moves)
    if not normalised:
        return 0
...
def download_eco_database(cache_path: str) -> pd.DataFrame:
    ...
    eco_df["eco_line_length"]  = eco_df["moves"].apply(count_eco_line_length)
...
def build_eco_lookup(eco_df: pd.DataFrame) -> dict:
    ...
    for _, row in eco_df.iterrows():
        lookup[row["moves_clean"]] = (
            row["eco_code"],
            row["name"],
            int(row["eco_line_length"]),
        )
    return lookup


def match_opening(opening_moves: str,
                  lookup: dict,
                  max_depth: int = 10) -> tuple[str, str, int]:
    tokens = str(opening_moves).strip().split()
...
def integrate(games_path: str, output_path: str) -> None:
    ...
    for moves in tqdm(games_df["opening_moves"], desc="Matching openings", unit=" games"):
        eco_code, opening_name, line_length = match_opening(str(moves), lookup)
        eco_codes.append(eco_code)
        opening_names.append(opening_name)
        eco_line_lengths.append(line_length)

    games_df["eco_code"]         = eco_codes
    games_df["opening_name"]     = opening_names
    games_df["eco_line_length"]  = eco_line_lengths
    ...
    print(games_df["rating_group"].value_counts().reindex(RATING_LABELS).to_string())

    print(f"\nECO line length distribution (matched games only):")
    matched_df = games_df[games_df["eco_code"] != "Unknown"]
    print(matched_df["eco_line_length"].describe().round(2).to_string())

    has_acpl = games_df["post_opening_acpl"].notna().sum() if "post_opening_acpl" in games_df.columns else 0
    print(f"\nGames with post-opening ACPL data : {has_acpl:,} "
          f"({100 * has_acpl / len(games_df):.1f}%)")
...
def main():
    parser = argparse.ArgumentParser(
        description="Integrate cleaned Lichess games with the ECO opening database."
    )
    parser.add_argument(
